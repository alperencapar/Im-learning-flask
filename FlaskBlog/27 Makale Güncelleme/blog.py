...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if "logged_in" in session:
            ...
            flash("Bu sayfayı görüntülemek için giriş yapın","alert alert-danger")
...
def delete(id):
    ...
    if result > 0:
        ...
        flash("Makale Yok veya Silmeye Yetkiniz Yok","alert alert-danger")
        return redirect(url_for("index"))
...
def update(id):
    if request.method == "GET":
        ...
        if result == 0:
            flash("Böyle bir Makale Bulunmuyor veya Yetkiniz Yok", "alert alert-danger")
            return redirect(url_for("index"))
        else:
            article = cursor.fetchone()
            form = ArticleForm()
        ...
        flash("Makale GÜncellendi","alert alert-success")
        return redirect(url_for("dashboard"))
